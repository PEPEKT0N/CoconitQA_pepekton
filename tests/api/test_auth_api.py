...
class TestAuthAPI:
    ...
    def test_register_and_login_user(self, api_manager: ApiManager, registered_user, login_data_user):
        ...
        """
        test_user, register_user_response = registered_user

        response = api_manager.auth_api.login_user(login_data_user, expected_status=201)
        response_data = response.json()

        assert "accessToken" in response_data, "Токен доступа отсутствует в ответе"
        assert response_data["user"]["email"] == register_user_response.email, "Email не совпадает"
    ...
    def test_create_user(self, super_admin, creation_user_data):
        ...
        assert response.get('roles') == [role.value for role in creation_user_data.roles]
    ...
    def test_get_user_by_locator(self, super_admin, creation_user_data):
        ...
        assert response_by_id.get('roles', []) == [role.value for role in creation_user_data.roles]
